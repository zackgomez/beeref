...
class BaseItemMixin(_BaseItemBase):
    def bee_scene(self) -> BeeGraphicsScene | None:
        """Return the scene cast to BeeGraphicsScene, or None."""
        scene = self.scene()
        if scene is None:
            return None
        return cast("BeeGraphicsScene", scene)

    def require_scene(self) -> BeeGraphicsScene:
        """Return the scene. Asserts the item is in a scene."""
        scene = self.bee_scene()
        assert scene is not None, f"{self} is not in a scene"
        return scene

    def bee_view(self) -> BeeGraphicsView | None:
        """Return the first view cast to BeeGraphicsView, or None."""
        scene = self.bee_scene()
        if scene is None:
            return None
        views = scene.views()
        if not views:
            return None
        return cast("BeeGraphicsView", views[0])

    def require_view(self) -> BeeGraphicsView:
